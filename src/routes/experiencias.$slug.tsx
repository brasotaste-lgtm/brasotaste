...
import { canonicalUrl, DEFAULT_OG_IMAGE, SITE_URL } from "@/lib/seo";
...
export const Route = createFileRoute("/experiencias/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const title = `${exp.title} | Experiência Braso Taste`;
    const url = canonicalUrl(`/experiencias/${exp.slug}`);
    return {
      meta: [
        { title },
        ...
        { name: "twitter:description", content: exp.text },
    ...
    },
    {
      ...
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Início", item: SITE_URL },
        {
          ...
          position: 2,
          name: "Experiências",
          item: canonicalUrl("/experiencias"),
        },
        ...
        <section className="relative isolate min-h-[350px] w-full overflow-hidden bg-brand-navy-deep text-white">
          ...
          <div className="relative z-10 mx-auto flex min-h-[350px] max-w-7xl flex-col justify-end px-5 pb-8 pt-24 sm:px-8 sm:pb-10 sm:pt-24">
            ...
            <div className="mt-4 flex items-center gap-4">
              ...
              <p className="text-[14px] tracking-[0.28em] uppercase text-brand-gold">
                {exp.sub}
              </p>
            </div>
          ...
          <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 lg:grid-cols-12 lg:gap-10">
            <Reveal className="lg:col-span-6">
              ...
              <div className="mt-6 rounded-sm border border-border/70 bg-card p-5">
                <p className="text-[14px] tracking-[0.24em] uppercase text-brand-gold">
                  Ideal para
                </p>
                <p className="mt-3 text-[18px] leading-relaxed text-foreground/85">
                  {exp.ideal}
                </p>
              </div>
            </Reveal>
            ...
            <Reveal delay={120} className="lg:col-span-6">
              ...
              <ul className="mt-6 space-y-5">
                {exp.highlights.map((h) => (
                  <li
                    key={h.title}
                    className="border-l-2 border-brand-gold/70 pl-5"
                  >
                    <p className="font-display text-xl font-medium text-brand-navy">
                      {h.title}
                    </p>
                    <p className="mt-2 text-[18px] leading-relaxed text-muted-foreground text-justify">
                      {h.text}
                    </p>
                  </li>
                ))}
        ...
        <section className="bg-brand-cream-deep py-8 sm:py-10 lg:py-8">
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            <Reveal className="max-w-2xl">
              ...
              <h2 className="mt-3 font-display text-3xl font-light leading-tight text-brand-navy sm:text-4xl">
                Uma operação{" "}
                <span className="italic text-brand-gold">completa</span>.
              </h2>
              <span className="gold-divider mt-4" />
              <p className="mt-6 text-[18px] leading-relaxed text-muted-foreground text-justify">
                Cuidamos de todos os detalhes para que você apenas viva o
                momento.
              </p>
            </Reveal>
              ...
              {exp.includes.map((item, i) => (
                <Reveal key={item} delay={i * 60}>
                  <li className="flex items-start gap-4 rounded-sm border border-border/70 bg-card p-5">
                    ...
                    <span className="text-[18px] leading-relaxed text-foreground/85">
                      {item}
                    </span>
                  </li>
                </Reveal>
        ...
        <section className="bg-background py-8 sm:py-10 lg:py-8">
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            <Reveal>
              ...
              <h2 className="mt-3 font-display text-3xl font-light leading-tight text-brand-navy sm:text-4xl">
                Explore outros{" "}
                <span className="italic text-brand-gold">formatos</span>.
              </h2>
              <span className="gold-divider mt-4" />
            </Reveal>
                ...
                <Reveal key={o.slug} delay={i * 80}>
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col px-6 py-6">
                      ...
                      <h3 className="mt-2 font-display text-xl font-medium text-brand-navy">
                        {o.title}
                      </h3>
